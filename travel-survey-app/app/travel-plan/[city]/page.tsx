...
function HotelRecommendations({ city }: { city: string }) {
  ...
  useEffect(() => {
    const fetchHotels = async () => {
      try {
        // Fetch hotels from Amadeus API via server-side proxy endpoint
        const response = await fetch(
          `/api/amadeus-hotels?city=${encodeURIComponent(city)}`
        );

        if (!response.ok) {
          throw new Error("Failed to fetch hotel recommendations from Amadeus API");
        }

        const data = await response.json();

        // Map Amadeus API response to HotelOption interface
        const mappedHotels: HotelOption[] = data.data.map((hotel: any) => ({
          id: hotel.hotel.hotelId,
          name: hotel.hotel.name,
          rating: hotel.hotel.rating ? parseFloat(hotel.hotel.rating) : 4.0, // Fallback rating if not provided
          price: hotel.offers[0]?.price?.total
            ? `$${hotel.offers[0].price.total}/night`
            : "$200/night", // Fallback price
          imageUrl: hotel.hotel.media?.[0]?.uri || "/api/placeholder/300/200", // Fallback image
          description:
            hotel.hotel.description?.text ||
            "A comfortable hotel with modern amenities in the city center.", // Fallback description
        }));
        ...
        const mockHotels = [
          {
            ...
            description:
              "Luxury hotel in the heart of downtown with amazing city views and premium amenities.",
          },
          {
            id: "hotel2",
            name: "Riverside Inn",
            ...
            imageUrl: "/api/placeholder/300/200",
            description:
              "Charming boutique hotel located near major attractions with complimentary breakfast.",
          },
          {
            id: "hotel3",
            name: "Urban Suites",
            ...
            imageUrl: "/api/placeholder/300/200",
            description:
              "Modern all-suite hotel with kitchenettes, perfect for extended stays and families.",
          },
        ];
      ...
      if (i < fullStars) {
        stars.push(
          <svg
            ...
            viewBox="0 0 20 20"
          >
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1cranial 4.488c-.528 1.2-1.68 2.4-3.312 3.192L19.2 32C21.68 30.8 23.68 28.728 24.4 25.6H22.928z" />
          </svg>
        );
      ...
      {hotels.map((hotel) => (
        <div key={hotel.id} className="bg-gray-700 rounded-lg overflow-hidden shadow-lg">
          ...
          <div className="p-4">
            ...
            <p className="text-gray-300 text-sm mb-4 blur-sm">{hotel.description}</p>
...
export default function TravelPlanPage() {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [backgroundImage, setBackgroundImage] = useState(""); // State for background image, initially empty
  ...
  const fetchCityImage = async (city: string) => {
    if (!city) {
      ...
      setBackgroundImage(""); // No default image
      return;
    }
    ...
    try {
      ...
      localStorage.setItem("backgroundImage", data.imageUrl); // Store the image URL
      console.log("Background image set to:", data.imageUrl);
    } catch (error) {
      console.error("Error fetching city image:", error);
      setBackgroundImage(""); // No default image on error
    }
  };
  ...
  useEffect(() => {
    ...
    // Check if user is logged in and has unblurred access
  ...
  const handleUnblur = () => {
    ...
    router.push(`/travel-packages/${city}`);
  };
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
            ...
            <p className={isBlurred ? "text-gray-300 leading-relaxed blur-sm" : "text-gray-300 leading-relaxed"}>
              {planData.local_food_recommendations}
            </p>
          </div>
        </div>
