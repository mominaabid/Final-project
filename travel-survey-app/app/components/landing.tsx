...
const Calendar = ({ onSelect, onClose }) => {
  ...
  return (
    <motion.div
      ...
      onClick={onClose}
    >
      <div className="bg-gray-800/90 backdrop-blur-md rounded-xl p-6 max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
...
export default function HomeClient() {
  ...
  const handlePlanNow = async () => {
    ...
    const payload = { city: country, start_date: startDate, end_date: endDate, travelers: travellers };
    ...
    try {
      ...
      if (data.description && data.activities?.length > 0) {
        localStorage.setItem("cityInfo", JSON.stringify({ ...data, ...payload }));
        localStorage.setItem("city", country);
        localStorage.setItem("start_date", startDate);
        localStorage.setItem("end_date", endDate);
        localStorage.setItem("travelers", travellers.toString());
        router.push(`/city/${encodeURIComponent(country)}`);
      } else {
          ...
          <div className="max-w-6xl mx-auto min-h-screen flex flex-col md:flex-row gap-8 items-center justify-center relative z-10">
            ...
            >
              ...
              <p className="text-lg leading-relaxed text-gray-100">
                Discover amazing destinations around the globe with our expertly curated travel packages. Whether you're seeking adventure in the
                mountains, relaxation on pristine beaches, or cultural experiences in historic cities, we have the perfect journey waiting for you. Our
                experienced team ensures that every detail of your trip is carefully planned, allowing you to focus on creating unforgettable memories.
              </p>
              <motion.button
